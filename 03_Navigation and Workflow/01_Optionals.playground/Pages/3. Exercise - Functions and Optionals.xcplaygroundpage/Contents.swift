...
func checkAge (_ checkUserInput: String) -> Int? {
    if let checkUserInput = Int(userInputAge) {
        print(checkUserInput)
        if  checkUserInput >= 18 {
            print("WELCOME!!")
...
func costOfItem(_ name: String) -> Double? {
    let nameStock = Array(stock.keys)
    for nam in nameStock {
        if name == nam && stock["\(nam)"] != 0 {
            print("The price of \(nam) is \(prices["\(nam)"])")
            return prices["\(nam)"]
        }
    }
    return nil
