...
print(calculateArea(x: 4, y: 6) ?? "Area is not Found!")
print(calculateArea(x: 4, y: -6) ?? "Area is not Found!")
...
print(add(x:4, y:5) ?? "Addition is not Found!")
print(add(x:4, y:nil) ?? "Addition is not Found!")
/*:
...
func createUser() -> User? {
    guard let firstNameTextField = firstNameTextField.text,
          let lastNameTextField = lastNameTextField.text,
          let ageTextField = ageTextField.text else {
        return nil
    }
    var newUser = User(firstName: firstNameTextField, lastName: firstNameTextField, age: ageTextField)
    return newUser
}
...
if let createUsers = createUser() {
    print("Hi!, I am \(createUsers.firstName) \(createUsers.lastName) and I am \(createUsers.age) old!")
}
